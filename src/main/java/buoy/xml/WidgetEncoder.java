...
/**
 * This class is used for serializing Widgets as XML. This allows user
 * interfaces to be saved in a persistent form, then reconstructed using the
 * WidgetDecoder class.
 * <p>
 * To use this class, simply call
 * {@link buoy.xml.WidgetEncoder#writeObject WidgetEncoder.writeObject()},
 * passing it the root Widget to save and an OutputStream to write the XML to.
 * It will save that Widget, along with all other objects referenced by it:
 * child Widgets, event listeners, models, etc.
 * <p>
 * The serialization is done through the java.beans.XMLEncoder class. It
 * therefore can encode any object which is a bean. An object which is not a
 * bean can also be saved if a PersistenceDelegate has been defined for it. The
 * buoy.xml.delegate package includes PersistenceDelegates for all of the
 * Widgets in the buoy.widget package. If you wish to define delegates for other
 * classes, you can do so by calling
 * {@link buoy.xml.WidgetEncoder#setPersistenceDelegate WidgetEncoder.setPersistenceDelegate()}.
 * If the class in question is an EventSource, the delegate should generally
 * subclass {@link EventSourceDelegate} and invoke
 * <code>initializeEventLinks()</code> from its <code>initialize()</code> method
 * so that its event listeners will be properly saved.
 * <p>
 * It is possible for XML files to contain localized text, where the actual
 * value to use is supplied from a ResourceBundle at decoding time. See
...
public class WidgetEncoder {
    ...
    /**
     * WidgetEncoder defines only static methods, and should never be
     * instantiated.
     */
    ...
    /**
     * Serialize an object hierarchy as XML. The OutputStream will be closed
     * once the XML is written.
     *
    ...
    /**
     * Serialize an object hierarchy as XML. If recoverable errors occur during
     * serialization, the listener will be notified of them. The OutputStream
     * will be closed once the XML is written.
     *
     * @param obj the object which forms the root of the hierarchy to save
     * @param out the OutputStream to write the XML to
    ...
    public static void writeObject(Object obj, OutputStream out, ExceptionListener listener) {
        ...
        for (String localized1 : WidgetLocalization.getAllLocalizedStrings()) {
            encoder.writeExpression(new Expression(localized1, WidgetLocalization.class, "getLocalizedString", new Object[]{localized1}));
        }
