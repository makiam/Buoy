...
/**
 * A BToolTip is a small floating window that appears in front of another
 * Widget. It contains a single line of text, and generally provides information
 * about the function of the Widget it is displayed over.
 * <p>
 * The user triggers a tool tip by placing the mouse pointer over a Widget and
 * leaving it there for a fixed amount of time. When this happens, the Widget
 * sends out a {@link buoy.event.ToolTipEvent ToolTipEvent}. The program should
 * respond to the event by calling either
 * {@link buoy.widget.BToolTip#show show()} or
 * {@link buoy.widget.BToolTip#processEvent processEvent()} to display the tool
 * tip.
 * <p>
 * The easiest way of doing this is to have the BToolTip listen for the event
 * directly. For example, you can set a fixed String as the tool tip for a
 * button by invoking:
 * <p>
 * <
 * pre>
 ...
 * </pre>
 * <p>
 * In some cases, you may want to do additional processing before the tool tip
 * is displayed. For example, you might want to customize the tool tip text
 * based on the mouse location, or exercise finer control over where the tool
 * tip appears. In these cases, your program should listen for the event and
 * handle it in the appropriate way.
 * <p>
 * As an example, the following code will display a tool tip over a BTable,
 * showing the row and column the mouse is currently pointing to:
 * <p>
 * <
 * pre>
 ...
 * </pre>
 * <p>
 * There can only be one tool tip showing at any time. It will automatically be
 * hidden as soon as a new tool tip is shown, or the mouse is moved away from
 * the Widget. You can call
 * {@link buoy.widget.BToolTip#getShowingToolTip getShowingToolTip()} to get the
 * currently displayed tool tip, or {@link buoy.widget.BToolTip#hide hide()} to
 * hide it.
 *
...
public class BToolTip extends Widget<JToolTip> {
    ...
    /**
     * Create the JToolTip which serves as this Widget's Component. This method
     ...
     *
     * @param event the event from which to determine where to display the tool
     * tip
     */
