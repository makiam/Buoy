 ...
 * tabs which provide various ways of choosing the color, such as selecting one
 * from a palette or specifying values for hue, saturation, and brightness.
 * <p>
 * BColorChooser can be used in two different ways. First, it can be added to a
 * container like any other Widget. This is useful when you want it to appear
 * inside of a window along with other Widgets.
 * <p>
 * Most often, however, BColorChooser is used in a modal dialog as a self
 * contained user interface element. To use it this way, you simply instantiate
 * a BColorChooser, set any properties, and then call
 * {@link buoy.widget.BColorChooser#showDialog showDialog()} to display it.
 *  <code>showDialog()</code> will automatically create a dialog, add the
 * BColorChooser, display it, and block until the user dismisses the dialog. You
 * can reuse a single BColorChooser by repeatedly calling
 * <code>showDialog()</code>.
 * <p>
 * In addition to the event types generated by all Widgets, BColorChoosers
...
public class BColorChooser extends Widget<JColorChooser> {
    ...
    /**
     * Create the JColorChooser which serves as this Widget's Component. This
    ...
    /**
     * Show a dialog containing this BColorChooser and block until the user
     * closes it. After this method returns, you can call
     * <code>getColor()</code> to determine what color was selected.
     *
     * @param parent the dialog's parent Widget (usually a WindowWidget). This
     * may be null.
     * @return true if the user clicked the OK button, false if they clicked the
     * cancel button
     */
