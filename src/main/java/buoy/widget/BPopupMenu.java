...
/**
 * A BPopupMenu is a WidgetContainer corresponding to a popup menu. It is
 * typically displayed in response to a
 * {@link buoy.event.WidgetMouseEvent WidgetMouseEvent} whose
 * <code>isPopupTrigger()</code> method returns true. The exact conditions which
 * represent a popup trigger are platform specific.
 * <p>
 ...
 * <p>
 * on the widget. This will automatically take care of showing the menu whenever
 * the user performs the appropriate action. Alternatively, you can write your
 * own code to listen for the event and call <code>show()</code> on the popup
 * menu. For example, you might want to disable certain menu items before
 * showing it based on the position of the mouse click.
 *
...
public class BPopupMenu extends WidgetContainer<JPopupMenu> implements MenuWidget {
    ...
    /**
     * Create the JPopupMenu which serves as this Widget's Component. This
     ...
     * <code>event.isPopupTrigger()</code> returns false, this method returns
     * without doing anything. Otherwise it shows the popup, determining the
     * appropriate location based on information stored in the event.
     * <p>
     * This version of show() is provided as a convenience. It allows you to
     * very easily add a popup menu to a Widget by invoking
     * <p>
     * <code>widget.addEventLink(WidgetMouseEvent.class, popup, "show");</code>
     *
