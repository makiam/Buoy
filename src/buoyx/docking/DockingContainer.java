 ...
 * {@link DockableWidget DockableWidgets}. The DockableWidgets are arranged
 * along one edge of the content Widget, divided into a set of tabs. The number
 * of tabs, the set of DockableWidgets in each tab, and the order in which they
 * appear are all configurable.
 * <p>
 * Once you create one or more DockingContainers and add DockableWidgets to
 * them, the user is free to rearrange them by dragging them with the mouse.
 * This includes reordering the Widgets in a tab, moving them between tabs,
 * moving a Widget from one DockingContainer to a different DockingContainer in
 * the same window, and detaching a Widget so that it appears in a separate
 * dialog.
 * <p>
 * Whenever the user performs a drag, a {@link DockingEvent} is dispatched to
 * report the event. When the drag is from one DockingContainer to another one,
 * both the source and target containers will dispatch events. For drags within
 * a single container, only one event is generated.
 *
...
public class DockingContainer extends WidgetContainer {
     ...
     * @param tabPosition this specifies which side of the content Widget the
     * DockableWidgets will be placed on
     */
     ...
     * @param indexInTab the position within the tab at which the Widget should
     * appear
     */
    ...
    /**
     * Get the index of the tab in which a DockableWidget appears. If it is not
     * a child of this container, -1 is returned.
     */
    ...
    /**
     * Get the position within its tab at which a DockableWidget appears. If it
     * is not a child of this container, -1 is returned.
     */
    ...
    /**
     * When all DockableWidgets are grouped into a single tab, it is possible to
     * save space by hiding the tab. This method gets whether that option is
     * enabled.
     */
    ...
    /**
     * When all DockableWidgets are grouped into a single tab, it is possible to
     * save space by hiding the tab. This method sets whether that option is
     * enabled.
     */
    ...
    /**
     * Reset the positions of the dividers within a single tab based on the
     * preferred sizes of the DockableWidgets it contains.
     */
    ...
    /**
     * Get the BSplitPane which separates the content Widget from the
     * DockableWidgets.
     */
