...
/**
 * A WindowWidget is a WidgetContainer corresponding to a window. This is an
 * abstract class, with subclasses for particular types of windows.
 *
...
public abstract class WindowWidget extends WidgetContainer {
    ...
    /**
     * Set the position and size of the window, then re-layout the window
     * contents.
     */
    ...
    /**
     * Select an appropriate size for the window, based on the preferred size of
    ...
    /**
     * Request that this window be brought to the front, so that it is displayed
     * over all other windows.
     * <p>
     * Note that the behavior of this method is highly platform dependent. It is
     * not guaranteed to work on all platforms. In addition, this method may or
    ...
    /**
     * Request that this window be sent to the back, so that it is displayed
    ...
    /**
     * Get the default button for this window. If the user presses the Return or
     * Enter key while the window has focus, it will be treated as if they had
     * clicked the default button (unless another Widget first consumes the
     * event). The default button is typically drawn differently to indicate its
    ...
    /**
     * Set the default button for this window. If the user presses the Return or
     * Enter key while the window has focus, it will be treated as if they had
     * clicked the default button (unless another Widget first consumes the
     * event). The default button is typically drawn differently to indicate its
     * special status. This may be null.
