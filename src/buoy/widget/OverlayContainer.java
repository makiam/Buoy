...
/**
 * OverlayContainer is a WidgetContainer which overlays its children on top of
 * each other. Every child Widget is sized to fill the entire container, and the
 * preferred size of the container is equal to the largest preferred size of any
 * of its children.
 * <p>
 * OverlayContainers are generally used to switch between several different
 * Widgets which all appear in the same position. When used this way, only one
 * of the Widgets will be visible at any time. The
 * {@link buoy.widget.OverlayContainer#setVisibleChild setVisibleChild()} method
 * provides an easy way to switch between the child Widgets by showing one and
 * hiding all the others.
 * <p>
 * Another use of OverlayContainer is to place a partially transparent Widget in
 * front of another one. This allows an "overlay" to be displayed at a
 * particular position, independent of the positions of other Widgets in the
 * window. An OverlayContainer can also be used to block mouse events from
 * reaching a Widget. Mouse events are always delivered to the topmost visible
 * Widget that has requested to receive them. Suppose that
 * <code>mainWidget</code> is a Widget (possibly a WidgetContainer with many
 * children). The following code will block all mouse events from reaching it
 * and its children:
 * <p>
 * <
 * pre>
 ...
 * <code>blocker.setVisible(true)</code> or
 * <code>blocker.setVisible(false)</code>. Note that we have added an event link
 * to <code>blocker</code> which listens for mouse events. If you do not do
 * this, it will ignore mouse events and let them pass through to the Widget
 * underneath.
 * <p>
 * In addition to the event types generated by all Widgets, OverlayContainers
...
public class OverlayContainer extends WidgetContainer {
     ...
     * @param index the index of the Widget within the container. Widgets with
     * lower indices are displayed in front of ones with higher indices.
     */
    ...
    /**
     * Add a Widget to this container. The new Widget will be placed in front of
     * all other Widgets in the container (at index 0).
     *
     * @param widget the Widget to add
     */
    ...
    /**
     * Set the i'th child Widget to be visible, and all others to be not
     * visible.
     */
    ...
    /**
     * Set a particular child Widget to be visible, and all others to be not
     * visible.
     */
