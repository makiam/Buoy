...
/**
 * This class is used for reconstructing user interfaces that were serialized as
 * XML by WidgetDecoder.
 * <p>
 * To use this class, simply create a new WidgetDecoder, passing it an input
 * stream from which it can read the XML file. You then can call
 * {@link buoy.xml.WidgetDecoder#getRootObject getRootObject()} to get the root
 * object of the hierarchy, or
 * {@link buoy.xml.WidgetDecoder#getObject getObject()} to look up any Widget in
 * the file by name.
 * <p>
 * Note that WidgetDecoder does <i>not</i> close the stream you pass to it, so
 * be sure to close the stream when you are done with it.
 *
...
public class WidgetDecoder {
     ...
     *
     * @param in the InputStream from which to read the XML
     * @param resources a ResourceBundle from which to obtain localized Strings.
    ...
    /**
     * Create a WidgetDecoder to reconstruct a Widget hierarchy from its XML
     * representation. If recoverable errors occur during reconstruction, the
     * listener will be notified of them.
     *
     * @param in the InputStream from which to read the XML
     ...
     * @return the specified object, or null if there was no object in the file
     * with that name
     */
    ...
    /**
     * This method is used during decoding. It records a named object into the
     * table of objects in the current file. If no file is currently being read,
     * this does nothing. There usually is no reason for you to call this
     * directly.
     */
