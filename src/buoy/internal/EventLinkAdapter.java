...
/**
 * This class handles interaction with the AWT event model. Its job is to
 * implement every listener interface, then generate and dispatch appropriate
 * event objects.
 *
...
public class EventLinkAdapter implements FocusListener, KeyListener, MouseListener,
    ...
    /**
     * This is called when a new event link is added to a Widget. It checks to
     * see whether this adapter has already been added as a listener for that
     * event type, and if not, it adds itself.
     */
    ...
    /**
     * Dispatch a Buoy event in response to an AWT event, then consume the
     * original event if the Buoy event was consumed.
     */
